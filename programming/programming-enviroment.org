...
*** WSL2-Arch linux share the folder with windows10/11
All Windows disk directories are in the /mnt directory of WSL.
#+begin_src shell
cd /mnt 
ls 
# print c/ d/ e/ l/
...
Use soft links to build a development environment synchronized with Windows under WSL.
#+begin_src shell
mkdir folder_name
cd folder_name
ln -s /mnt/e/folder_name folder_name
#+end_src
...
+ Macos
  #+begin_src 
  wget https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-arm64.sh -O Miniconda3.sh
...
Conda init.
#+begin_src shell
~/miniconda3/bin/conda init zsh  # 如果用 zsh
source ~/.zshrc  
#+end_src

Disbale =base= enviroment.
#+begin_src shell
...
Install =mamba=.
#+begin_src shell
conda create -n conda_tools -c conda-forge mamba
conda activate conda_tools
#+end_src

Using =mamba= replace =conda=.
#+begin_src shell
mamba create -n new_env python=3.10 
#+end_src

When using mamba, need to activate the =conda_tools= environment first.
#+begin_src shell
conda activate conda_tools
#+end_src
